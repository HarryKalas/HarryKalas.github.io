...
function showPlay(playText) {
	...
	case "bottom" :
		...
		break;
	case "game" :
		if (selectNodes(source, "..", playText).snapshotItem(0).getAttribute("inning_state") == "Bottom") {
			TeamInfo = document.getElementById("home");
		} else {
			TeamInfo = document.getElementById("away");
		}
...
function SecondaryPlay(Play, Prefix) {
	...
	if (!SecondBox) { 
		if ((Plays[Ptr].indexOf(" error") >= 0)) {
			...
			if (!ans) { die; }
			window.open(gameFolder + "/game_events.xml");
			alert("3 " + TeamInfo.Index);
			for (Idx = 1; Idx <=9; Idx++) {  
				alert("4 " + PlayerFull[TeamInfo.Index][Idx]);
			}
		}
		return; 
	}
...
function clearMessage(Msg) {
	...
	if (document.getElementById("MessageDiv").innerHTML == '') {
		...
		alert("clearMessage " + document.getElementById("MessageDiv").innerHTML);
	}
}
