...
//	if it has, if updates the last date [1] and does a new pull [2]
	...
	if (theFile[1] != null) {
		...
		try {
			...
			//IF IT FAILED TO LOAD THE HEADER, MOVE ON A TRY THE BODY
...
alert("Runner out play: ", ActionPlay);
...
document.getElementById("LastSound").innerHTML = AudioQueue.length;
               return;
...
console.log("action: ", selectNodes(pitchSource, "game/action").snapshotItem(0).getAttribute("des"));
		setTimeout("console.log('action from pitches');LastPlay = LoadPlays(LastPlay);", 1000); // get the call a second later, sometimes it shows up a little after in this place
	}
...
console.log("atbat: ", selectNodes(pitchSource, "game/atbat").snapshotItem(0).getAttribute("des"));
		setTimeout("console.log('atbat from pitches');LastPlay = LoadPlays(LastPlay);", 1000); // get the call a second later, sometimes it shows up a little after in this place
	}
}
